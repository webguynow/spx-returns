...
class ReturnsList extends Component {
    ...
    render() { 
        return ( 
            <div style={divStyle}>
                <div style={{'width': '80%'}}>
                    <div>Choose the Years from the Slider</div>
                    <Range allowCross={false} min={this.state.initRangeVal[0]} max={this.state.initRangeVal[1]}  defaultValue={this.state.yearRange} onAfterChange={this.onRangeChange} />
                    Returns from Year-Range <input type="text" value={this.state.yearRange[0]} /> thru <input type="text" value={this.state.yearRange[1]} /> 
                    <div>See the Yearly and Cumulative Returns</div>
                </div>                    
                <table>
                    <thead>
                    <tr>
                    ...
                    <tbody>
                            { this.orderFilterYears()
                                .map((acc => x => { acc = acc * (1 + x.totalReturn/100); return ({ "year":x.year, "totalReturn":x.totalReturn, "cumulativeReturn":round((acc-1)*100, 2) }) } )(1.0))
                                .map(h => 
                                (<tr><td>{h.year}</td><td className={h.totalReturn<0 ? 'colorRed' : 'colorBlack'}>{h.totalReturn}</td><td>{h.cumulativeReturn}</td></tr>)) }
